...
class MainApp:
    def __init__(self, master=None):
        ...
        self.username = ttk.Entry(self.setupFrame, width=20, textvariable='abryandog@gmail.com')
        self.username.grid(column='0', row='0')
        self.username.insert(tk.END, 'abryandog@gmail.com')
        self.passcode = ttk.Entry(self.setupFrame, show="*", width=20, textvariable='And123rew456')
        self.passcode.grid(column='1', row='0')
        self.passcode.insert(tk.END, 'And123rew456')
        ...
        self.submitButton.configure(style='Toolbutton', text='Submit Now')
        self.submitButton.grid(column='1', pady='5', row='4', sticky='s')

        self.submitAuto = ttk.Button(self.setupFrame, command=lambda: [self.autoBC()])
        self.submitAuto.configure(style='Toolbutton', text='Auto')
        self.submitAuto.grid(column='1', pady=5 , row='3', sticky='s')
    ...
    def submitBC(self, event=None):
        # TODO CONNECT TEXT AND PHOTO BOXES

        # TODO SET UP POSTING FROM SERVER
        facebookSelenium.startFBLogin(self.username.get(), self.passcode.get(), self.descriptionText.get("1.0", tk.END), self.filepath)

    def autoBC(self, event=None):
        try:
    ...
    def uploadBC(self, event=None):
        ...
        img_pil = Image.open(self.filepath)
        width, height = img_pil.size
        print(width, height)
        self.imgTK = ImageTk.PhotoImage(img_pil)
